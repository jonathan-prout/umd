...
class mysql(generic.IInfoSourceMixIn):
	...
	def onXPointChange( self, source, dest, levels ):
		...
		with self.sqlLock:
			for lvl in levels:
				cmd = "SELECT `PRIMARY`  FROM `status` WHERE `matrixid` = {} AND `output` = {} AND `levels` LIKE '{}';".format(self.prefsDict["mtxId"], dest, lvl)
				#print self.name + " " + cmd
				try:
				...
				if primary:
					...
					cmd = "INSERT INTO `matrix`.`status` (`PRIMARY`, `matrixid`, `input`, `output`, `levels`, `time`) VALUES (NULL, '{}', '{}', '{}', '{}', CURRENT_TIMESTAMP);".format(self.prefsDict["mtxId"], source, dest, lvl)
				#print self.name + " " + cmd
				self.qselect(cmd)
			self.db.commit()
